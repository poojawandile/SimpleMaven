...
public class MainServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String strNumericals = request.getParameter("numericals");
		...
		if (!strPrimeElements.equalsIgnoreCase("")) {
			...
			// check if strFibElements is a valid number if true set the result in string in
			// Fibseries if false set the error message in the string.
			// In BP write method to validate if the num is correct, in BP write a method to
			// convert int arr to str, change the method generateFibseries
			// to return string.
		...
		if (!strFibElements.equalsIgnoreCase("")) {
			...
			// System.out.println("In side Myservlet Generated Fib
			// servies>>"+Arrays.toString(arr));
		...
		if (!strNumbersToBeSorted.equalsIgnoreCase("")) {
			...
			int[] numbers = Arrays.asList(strNumbersToBeSorted.split(",")).stream().map(String::trim)
		...
		if (!strNumericals.equalsIgnoreCase("")) {
			System.out.println(" In side Myservlet strNumericals >>" + strNumericals);

			String strNumInWords = convertor.convertNumberToText(strNumericals);
			System.out.println(" In side Myservlet strNumInWords >>" + strNumInWords);
			request.getSession().setAttribute("numInWords", strNumInWords);
			response.sendRedirect("index.jsp");
		}
